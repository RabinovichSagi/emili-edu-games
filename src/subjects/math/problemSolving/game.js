...
function normalizeConfig(cfg) {
  ...
  n.worksheetProblems = clamp(Number(n.worksheetProblems) || 10, 6, 12);
...
function buildWorksheetProblems(cfg) {
  const templates = cfg.enabledPatternIds.flatMap((id) => TEMPLATES_BY_PATTERN[id] || []);
  const pool = templates.length ? templates : ALL_PROBLEM_TEMPLATES;
  const shuffled = [...pool].sort(() => Math.random() - 0.5);
  const out = [];
  for (let i = 0; i < cfg.worksheetProblems; i++) {
    out.push(generateProblemFromTemplate(shuffled[i % shuffled.length], cfg.difficulty));
  }
  return out;
}
...
export const MathProblemSolvingGame = {
  ...
  render({ mount, store, router, screen }) {
    ...
    if (initialWorksheet) state.worksheet = buildWorksheetProblems(state.cfg);
    ...
    function buildWorksheet() {
      state.worksheet = buildWorksheetProblems(state.cfg);
      ...
      return el("div", { class: "list" }, [
        ...
        el("div", { class: "card" }, [
          el("div", { class: "row" }, [
            ...
            numberSelect("תרגילים בדף", state.cfg.worksheetProblems, [6, 8, 10, 12], (v) => saveCfg({ worksheetProblems: v })),
      ...
      return el("div", { class: "worksheetWrap" }, [
        ...
        el("section", { class: "worksheetPage" }, [
          el("div", { class: "worksheetHeader" }, [
            el("div", {}, [
              el("h2", { text: "בעיות מילוליות בחשבון" }),
              el("p", { text: `שם: ____________   תאריך: ${date}` }),
            ]),
            el("div", { class: "worksheetBadge", text: "משימת חשיבה" }),
          ]),
          el("ol", { class: "worksheetProblems" }, state.worksheet.map((problem) =>
            el("li", {}, [
              el("div", { class: "worksheetStory", text: problem.text }),
